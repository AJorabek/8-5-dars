...
function Card({ id, description, name, price, deleted, type }: ICard) {
  ...
  const handleDelete = () => {
    fetch(`https://auth-rg69.onrender.com/api/products/${id}`, {
      ...
      .then(() => {
        type && type.setData((e: any) => e.filter((e: any) => e.id !== id));
      })
      .catch((e) => {
        throw e;
      });
  ...
  return (
    <div
      ...
      className={
        "p-5 rounded-xl max-w-[250px] w-full bg-slate-50 grid gap-2 shadow border border-slate-100 cursor-pointer hover:scale-105 transition-all"
      }
    >
